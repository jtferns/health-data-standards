class Record
  ...
  def self.update_or_create(data, options={})
    if (data.medical_record_number.nil? || data.medical_record_number.empty?) && options[:generate_mrn]
       data.medical_record_number = Record.generate_mrn_number
       data.save!
       data
    else  
  ...
  def self.generate_mrn_number()
     "#{Time.now.to_i}_#{rand(10000000)}"
  end
